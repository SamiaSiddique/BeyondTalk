    ...
    newUser.save((err, addedUser) => {
      if (err) {
        ...
        if (role === "customer") {
          ...
          customer.save((err, resgisterCustomer) => {
            if (err) {
              return res.status(500).json({
                err: err,
                registered: false
              });
            } else if (resgisterCustomer) {
              return res.status(200).json({
                err: null,
                registered: true,
                customerId: resgisterCustomer.userId
              });
            }
          });
          ...
          employee.save((err, resgisterEmployee) => {
            if (err) {
              return res.status(500).json({
                err: err,
                registered: false
              });
            } else if (resgisterEmployee) {
              return res.status(200).json({
                err: null,
                registered: true,
                employeeeId: resgisterEmployee.userId
              });
            }
          });
        } else if (role === "admin") {
...
const ForgotPassword = async (req, res) => {
  try {
    ...
    const info = await transporter.sendMail({
      from: "BeyondTalk <info@beyondtalk.com>", // sender address
      to: email, // list of receivers
      subject: "BeyondTalk | Reset You Account Password", // Subject line
