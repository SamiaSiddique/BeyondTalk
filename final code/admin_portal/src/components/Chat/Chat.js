...
class MyChat extends React.Component {
  ...
  FetchConversationDetails = (selectedContact) => {
    ...
    sb.GroupChannel.getChannel(selectedContact.channel.url, (channel, err) => {
      if (err) {
        ...
        console.log("My Selected User ID is ", selectedContact.user.nickname);
        if (prevMessageListQuery.hasMore) {
          prevMessageListQuery.load(LIMIT, isReverse, (messages, error) => {
            if (error) {
              console.log("Error while fetching conversations: ", error);
              ...
              this.setState({
                ...
                selectedUserId: selectedContact.user.userId,
                selectedUserName: selectedContact.user.nickname,
                loader: false,
                conversation,
              });
              if (this.ConversationScrollBar) {
      ...
      <div className="gx-chat-box">
        {this.state.conversation === null ? (
          <div className="gx-comment-box">
            ...
            <h1 className="gx-text-muted gx-d-block gx-d-lg-none">
              {"Select User Chat"}
            </h1>
            {/* <Button
  ...
  /* All Function to Handle Audio and Video Call Start */

  callWithVideo = (video) => {
    const config = { audio: true, video };
    const isCaller = true;
    const friendID =
      this.state.selectedUserId + "," + this.state.selectedUserName;
    const friendName = this.state.selectedUserName;
    let message = "";
    if (video) {
      message = "📞 Video Call";
    } else {
    ...
    this.props.globalstartcall(isCaller, friendID, config, friendName);
  };

  /* All Function to Handle Audio and Video Call End  */
